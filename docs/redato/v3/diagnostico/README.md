...
**Atualizado:** 2026-05-04 · Fase 5A.1 (mapeamento livros — rascunho)
...
| **5A.2** Mapeamento descritores → BNCC | 🟡 2026-05-04 (pipeline pronto, geração pendente) | Pipeline LLM (mapeador_bncc.py) + catálogo BNCC EM-LP completo (54 habilidades EM13LP01-LP54) + helper runtime + 4ª seção nos cards de lacuna do MapaCognitivo. JSON committado como **placeholder** (`status: "nao_gerado_ainda"`) — Daniel roda localmente com `OPENAI_API_KEY` pra preencher. Custo estimado ~$0.40, latência ~3min. Detalhes em [`HOWTO_mapeamento_bncc.md`](HOWTO_mapeamento_bncc.md). |
